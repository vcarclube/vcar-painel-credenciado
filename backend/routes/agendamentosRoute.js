...
router.post('/agendar', validateToken, async (req, res) => {
  try {
    const { idPontoAtendimento, idSocio, idSocioVeiculo, data, hora, motivo, pecaPorContaDoSocio } = req.body;
    
    if (!idPontoAtendimento || !idSocio || !idSocioVeiculo || !data || !hora || !motivo || !pecaPorContaDoSocio) {
      return res.status(400).json({ 
        message: 'Dados incompletos' 
      });
    }

    const idSocioVeiculoAgendaGenerated = await Utils.generateUUID();
    ...
    let servicoReserva = await Utils.getServicoByNameReserva();
    let motivacaoTrocaDeOleo = "TROCA DE ÓLEO";
    let valorLitro = 40.0;
    let valorFiltro = 14.0;
    ...
    await Utils.notificarPontoAtendimento({
      idPontoAtendimento,
      titulo: 'Agendamento',
      conteudo: `Foi feito um agendamento do sócio ${socio.Nome} no ${pontoAtendimento.Descricao} para ${Utils.formatDateString(data)} ás ${hora}. Motivo: ${motivo}`,
      lido: false
    });
    ...
    await Utils.notificarWhatsapp({
      ...
      message: `
📆 Olá, ${socio.Nome}👋, Informamos que seu serviço foi agendado pela oficina.
...
🕐 *Hora*: ${hora}
    ...
    await Utils.notificarFirebaseCloudMessaging({
      ...
      title: 'Agendamento',
      body: `Olá ${socio.Nome}👋, Informamos que seu serviço foi agendado pela ${pontoAtendimento.Descricao} para ${Utils.formatDateString(data)} ás ${hora}.`
    });

    let servicoSelecionado = null;

    switch (socioVeiculo.TipoVeiculo){
        case "PASSEIO":
            servicoSelecionado = await Utils.getServicoTrocaDeOleoByVeiculoTipo(motivacaoTrocaDeOleo, "PASSEIO");
            break;
        case "SUV":
            servicoSelecionado = await Utils.getServicoTrocaDeOleoByVeiculoTipo(motivacaoTrocaDeOleo, "SUV");
            break;
        case "CAMINHONETE":
            servicoSelecionado = await Utils.getServicoTrocaDeOleoByVeiculoTipo(motivacaoTrocaDeOleo, "CAMINHONETE");
            break;
    }

    let valorAdicional = 0.0;

    if(pecaPorContaDoSocio == "N"){
        let capacidadeVeiculoSocio = await Utils.getSocioVeiculoCapacidadeLitros(socioVeiculo.IdSocioVeiculo);
        valorAdicional = (valorLitro * capacidadeVeiculoSocio?.Litragem) + valorFiltro;
    }
